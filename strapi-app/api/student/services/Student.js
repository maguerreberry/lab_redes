...
module.exports = {
  ...
  add: async (values) => {
    ...
    const entry = await Student.create(data);

    // Create relational data and return the entry.
    return Student.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Student.associations.map(a => a.alias));
    const data = _.omit(values, Student.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Student.updateOne(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Student.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Student.attributes).reduce((acc, curr) => {
      switch (Student.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
